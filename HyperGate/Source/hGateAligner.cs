...
{
    [KSPModule("HyperGate Aligner")]
    public class hGateAlignerModule : PartModule
    {
        static List<hGateAlignerModule> availablehGateAligner = new List<hGateAlignerModule>();
        ...
        protected hGateState state = hGateState.OFFLINE;
        private double activationTime = 0;
        protected FXGroup driveSound;

        /**
         * \brief HyperGate name (displayed in the GUI)
         * This is the name of the currently selected HyperGate. It is updated from the Next function,
         * which will go to the next MISALIGNED on the list.
         * \note this variable is not actually used by the mod, it is only for the GUI
         */
        [KSPField(guiActive = true, guiActiveEditor = true, guiName = "HyperGate", isPersistant = true)]
        private string hGateName = hGateSelector.NO_TARGET;
